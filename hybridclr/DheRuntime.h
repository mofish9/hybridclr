...
{
    // The DHE-lite binary MV format is intentionally small and immutable. It
    // carries the current method tokens; IL and metadata remain in the
    // homologous DLL loaded by the existing runtime path.
    ...
    struct MetaVersionData
    {
        ...
        uint32_t flags = 0;
        Sha256Digest baselineAssemblyHash{};
        Sha256Digest currentAssemblyHash{};
        std::vector<uint32_t> changedMethodTokens;
    };
